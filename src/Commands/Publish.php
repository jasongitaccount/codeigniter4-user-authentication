<?php namespace Citools\Auth\Commands;
...
class Publish extends BaseCommand
{
    ...
    /**
     * The path to Citools\Auth\src directory.
     *
     * @var string
     */
    ...
    protected function publishModels()
    {
        ...
        foreach ($models as $model)
        {
            ...
            $content = $this->replaceNamespace($content, 'Citools\Auth\Models', 'Models');
    ...
    protected function publishEntities()
    {
        ...
        $content = file_get_contents($path);
        $content = $this->replaceNamespace($content, 'Citools\Auth\Entities', 'Entities');
    ...
    protected function publishController()
    {
        ...
        $content = file_get_contents($path);
        $content = $this->replaceNamespace($content, 'Citools\Auth\Controllers', 'Controllers');
    ...
    protected function publishView($view, string $prefix = '')
    {
        ...
        $content = file_get_contents($path);
        $content = str_replace('Citools\Auth\Views', $namespace.'\Auth', $content);
    ...
    protected function publishFilters()
    {
        ...
        foreach ($filters as $filter)
        {
            ...
            $content = file_get_contents($path);
            $content = $this->replaceNamespace($content, 'Citools\Auth\Filters', 'Filters');
    ...
    protected function publishMigration()
    {
        ...
        foreach ($map as $file)
        {
            ...
            $content = $this->replaceNamespace($content, 'Citools\Auth\Database\Migrations', 'Database\Migrations');
    ...
    protected function publishConfig()
    {
        ...
        $content = file_get_contents($path);
        $content = str_replace('namespace Citools\Auth\Config', "namespace Config", $content);
        $content = str_replace("use CodeIgniter\Config\BaseConfig;\n", '', $content);
        $content = str_replace('extends BaseConfig', "extends \Citools\Auth\Config\Auth", $content);

        // are we also changing the views?
        if ($this->viewsPublished)
        {
            $namespace = defined('APP_NAMESPACE') ? APP_NAMESPACE : 'App';
            $content = str_replace('Citools\Auth\Views', $namespace . '\Views', $content);
        }
    ...
    /**
     * Replaces the Citools\Auth namespace in the published
