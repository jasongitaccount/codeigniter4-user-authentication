...
Citools:Auth uses its own views by default, but you may want to update these in order to change
...
Here are the default values:
	...
	public $views = [
		...
		'emailActivation' => 'Citools\Auth\Views\emails\activation',
	];

Every view is wrapped in Citools:Auth's default layout, which can also be changed by modifying
the `$viewLayout` property in the same config file:

	public $viewLayout = 'Citools\Auth\Views\layout';
...
Next you need to let the `UserModel` know about these additional fields. Citools:Auth uses
...
use Citools\Auth\Models\UserModel as CitoolsModel;

class UserModel extends CitoolsModel
{
...
> **Notice** The new model extends Citools's version, so all the other necessary properties and methods are still available.
...
use Citools\Auth\Models\User as CitoolsUser;

class User extends CitoolsUser
{
    /**
     * Default attributes.
     * @var array
